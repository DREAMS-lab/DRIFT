...
def output_collector():
    ...
    while fuzzing:
        try:
            # dumb temporary fix
            data = open(f"{logpath}fuzzer-master.log","rb").read().decode('latin')
            paths = data.split("(")[-1].split(" total")[0] 
            data2 = open(f"{logpath}fuzzer-master/fuzzer_stats", "rb").read().decode('latin')
            execs = data2.split('execs_per_sec')[1].split(':')[1].split('paths_total')[0].strip().split("\n")[0]
...
if __name__ == '__main__':
    start_monitor_fuzzing("/DRIFT/workdir/phuzwork/")
