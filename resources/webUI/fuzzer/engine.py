...
def start_mavlink_AFL():
    ...
    process = subprocess.Popen(['/DRIFT/webUI/examples/mavlink_afl.sh'])


def start_custom_AFL(cbpath, dict, argsp):
    global process

    bexec = "/DRIFT/webUI/examples/custom_afl.sh"

    print(f"[*] Starting AFL for custom binary at {cbpath} with dict {dict} and args {argsp}")
    bashcode = open("/DRIFT/webUI/examples/custom.sh", "r").read()
    ...
    process = subprocess.Popen(['/DRIFT/webUI/examples/custom_afl.sh'])
...
def start_fuzzing(fpath, fafl_cores, ffirst_crash, fno_dictionary, drillers):
    ...
    seeds = [b"/DRIFT/webUI/fuzzer/seeds"]
